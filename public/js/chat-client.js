/**
 * Production-ready ChatClient
 * - Fixed HTML escaping XSS vulnerability
 * - Automatic WSS/WS protocol selection based on host security context
 * - Encapsulated instance scope
 */
(() => {
  class ChatClient {
    constructor(options = {}) {
    ...
    createWidget() {
      ...
      widget.innerHTML = `
        <div id="chat-toggle" class="chat-toggle" role="button" aria-label="Toggle chat">
          💬
    ...
    async loadUserChats() {
      try {
        ...
        if (res.ok) {
          ...
          const openChats = data.chats ? data.chats.filter(c => ['open', 'in-progress'].includes(c.status)) : [];
    ...
    async createNewChat(category = 'general') {
      try {
        ...
        if (data.success) {
          ...
          this.showStatus('New chat created. Waiting for support...', 'offline');
        }
      } catch (e) {
    ...
    async connectWebSocket() {
      ...
      // Enforce WSS over secure connections (HTTPS)
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      const wsUrl = `${protocol}//${window.location.host}${this.wsBase}/${encodeURIComponent(this.conversationId)}`;
      ...
      this.ws.onclose = () => {
        ...
        setTimeout(() => {
          if (this.conversationId) this.connectWebSocket();
        }, 3000);
      };
    }
    ...
    async loadChatMessages() {
      ...
      try {
        const res = await fetch(`${this.apiBase}/chat/${encodeURIComponent(this.conversationId)}`, {
          credentials: 'same-origin'
        });
        const data = await res.json();
        if (data.success && Array.isArray(data.chat.messages)) {
    ...
    appendMessage(message) {
      ...
      msgDiv.className = `chat-message ${this.escapeHtml(message.role || 'admin')}`;
      msgDiv.innerHTML = `
        <div class="chat-message-bubble">${this.escapeHtml(message.content || '')}</div>
        <div class="chat-message-time">${new Date(message.timestamp || Date.now()).toLocaleTimeString()}</div>
      `;
    ...
    async sendMessage() {
      ...
      try {
        ...
        const res = await fetch(`${this.apiBase}/chat/${encodeURIComponent(this.conversationId)}/message`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function initChat(userEmail = '') {
    if (document.getElementById('chat-widget') || instance) return;
    instance = new ChatClient({ userEmail });
  }
