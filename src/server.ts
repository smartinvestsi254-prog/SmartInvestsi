...
// Previously, any request could set itself as admin via the `x-admin` header,
// `req.body.admin`, or `req.user.admin` — a critical privilege escalation /
// authentication bypass flaw. This is now REMOVED. Admin status is ONLY derived
// from a verified JWT payload (see verifyTokenFromReq above).
// The middleware below is intentionally a no-op that forwards the request.
app.use((req, _res, next) => {
  // Admin status is determined exclusively by the verified JWT payload.
  // Ignore any client-supplied admin hints for security.
  const { x_admin, ...rest } = req.headers as any;
  void rest;
  void x_admin;
  next();
});
