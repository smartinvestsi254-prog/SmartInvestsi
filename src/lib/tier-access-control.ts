...
export async function getUserTier(userEmail: string): Promise<SubscriptionTier> {
  try {
    // PRIORITY 1: Check active Subscriptions (new model) - highest tier wins
    ...
    if (activeSubs.length > 0) {
      ...
      return tierMap[activeSubs[0].plan] || SubscriptionTier.FREE;
    }

    // PRIORITY 2: Fallback to User.subscriptionTier (legacy)
...
// ... rest of functions unchanged (checkFeatureAccess, etc.)
